...
app.post('/details', (req, res) => {
  query('UPDATE product VALUES ? WHERE id=1', [res.body])
    .then((rows) => {
      res.json(rows[0]);
      res.end();
    })
...
app.post('/updated-account', ensureAuthenticated, (req, res) => {
  ...
  const { username, firstname, lastname, currentPass, newPass, confirmPass } =
    req.body;
...
app.post('/create-payment-intent', (req, res) => {
  const calTotal = (qty, price, tip) => (qty * price + Number(tip ? tip : 0)).toFixed(2);
  ...
  query('SELECT * FROM product')
    ...
    .then((paymentIntent) => {
      ...
      const ticketId = `ypstl_${firstname}_${lastname}_${
        paymentIntent.id.split('_')[1]
      }_${ticketCount}`;
    ...
    .then((_) => {
      const queries = [
        query(
          `INSERT INTO waivers(user_name, contact_name, relation, phone, signature, transaction_id)
            VALUES ($1, $2, $3, $4, $5, $6)`,
          [
            userName,
            waiverData.eName,
            waiverData.eRelation,
            waiverData.eNum,
      ...
      if (songReq.replace(/\s/g, '').length)
        queries.push(
          query('INSERT INTO song_req(transaction_id, name) VALUES($1, $2)', [
            transactionId,
            songReq,
          ])
        );
    ...
    .then((_) => {
      ...
      const rows = [];
      additionalTickets.forEach((i) => rows.push([i, transactionId]));
...
app.post('/transaction', ensureAuthenticated, (req, res) => {
  ...
  query('SELECT * FROM transactions WHERE ticket_id=$1 AND purchase_confirmed=TRUE', [ticketId])
    .then((rows) => {
      if (!rows.length) return res.json({ error: 'User not found' }).end();
      ...
      return query('SELECT * FROM extra_tickets WHERE buyer_id=$1', [ticketData.id]).then(
        (exRows) => {
...
app.get('/transactions', ensureAuthenticated, (req, res) => {
  const queries = [
    query(`SELECT qty as ticket_limit,
...
app.get('/song-requests', ensureAuthenticated, (req, res) => {
  query(
    'SELECT s.transaction_id as id, s.name as song, t.firstname, t.lastname FROM song_req s JOIN transactions t on s.transaction_id = t.id'
  )
    .then((results) => {
      console.log({ results });
...
app.post('/check-in', ensureAuthenticated, (req, res) => {
  ...
  Promise.all(queries)
    .then((rows) => {
      const updatedIds = [];
      rows.forEach((i) => updatedIds.push(i[0].id));
