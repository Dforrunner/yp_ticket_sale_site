...
const SongReqTab = ({ index, setTab, data, activeTab }) => {
  ...
  const handleSearch = (str) => {
    if (!str) return setRows(data);
    const filteredRows = fixedData.filter((i) =>
      `${i.firstname} ${i.lastname} ${i.song}`.includes(str)
    );
    setRows(filteredRows);
  };
    ...
    <div className={`${slideAnimation(index, prevTab)}  w-full md:mt-10`}>
      <div className='flex flex-col p-2'>
        {accessLevel === 2 && (
          <div className='flex flex-col mx-1 my-3 bg-[#252525] justify-between items-center p-5 rounded'>
            ...
            <p className='text-center text-sm text-gray-400 mt-2'>
              Note: Fees were not taken out for Venmo transactions to show accurate sales
            </p>
          </div>
        )}
          ...
          <div className='flex flex-col mx-1 my-3 bg-[#252525] justify-between items-center p-5 rounded'>
            ...
            <table className='w-[200px] my-5'>
              <tbody>
                ...
                <tr>
                  <td>Available:</td>
                  <td className='text-3xl text-green-200 float-right'>
                    {ticketLimit - soldTickets}
                  </td>
                </tr>
              </tbody>
            </table>
            ...
            <p className='text-center text-sm text-gray-400 mt-2'>
              Note: I subtracted 4 to save our spots
            </p>
          </div>
        )}
        ...
        <div className='sticky bottom-0 mb-10 mt-3'>
          <div
            className='flex justify-center items-center mx-1 bg-[#3071BB]
        ...
        <div className='sticky bottom-0 mb-10 mt-3'>
          ...
          >
            <button
              className='w-full h-full cursor-pointer bg-[#09D4CE]'
              type='button'
              onClick={() => handleTab(6)}
            >
              View Song Requests
...
const CheckinTab = ({ index, prevTab, setTab, scanData = [], setScanData }) => {
  ...
  const handleSubmit = (e) => {
    ...
    scanData.forEach((i) => {
      if (checkedUsers.includes(i.id) && !i.checked_in) usersToCheckIn.push(i);
    });

    if (!usersToCheckIn.length) return setMsg('Select a new customer to check-in');
    setMsg('');
    ...
    fetch('/api/check-in', {
      ...
      .then((res) => {
        ...
        scanData.forEach((i) => {
          if (checkedUsers.includes(i.id)) updatedData.push({ ...i, checked_in: true });
    ...
    fetch('/api/transaction', {
      ...
      .then((res) => {
        ...
        if (res.additionalTickets && res.additionalTickets.length)
          res.additionalTickets.forEach((i) => users.push({ mainBuyer: false, ...i }));
  ...
  return (
    <div className={`${slideAnimation(index, prevTab)} w-full md:mt-10 h-screen `}>
      <div className='flex justify-center items-center h-[84%] overflow-hidden'>
        {index === activeTab && <QrScanner delay={100} onError={handleError} onScan={handleScan} />}
      </div>
      ...
      <form onSubmit={handleSubmit} className='mt-5'>
        ...
        <div className='flex flex-col p-5 w-full'>
          <CustomInput
            ...
            error={usernameErr}
          />
          <CustomInput label='First Name' name='firstname' defaultValue={user.firstname} />
          <CustomInput label='Last Name' name='lastname' defaultValue={user.lastname} />
        </div>
        ...
        <div className='flex flex-col p-5 w-full'>
          ...
          {accessLevel === 2 && (
            <CustomInput label='New Password (Optional)' name='newPass' type='password' />
          )}
  ...
  return (
    <div className={`${slideAnimation(index, prevTab)} w-full`}>
      <div className='flex flex-col justify-start items-center'>
        ...
        <List
          className='w-[90%] rounded bg-gray-300 text-black overflow-hidden'
          style={{ padding: 0 }}
        >
          <CustomListItem
            ...
            onclick={() => setTab(3)}
          />
          <CustomListItem icon={<LogoutIcon />} text='Logout' onclick={handleLogout} />
        </List>
      </div>
    </div>
  );
    ...
    <div className='min-h-screen overflow-hidden text-white md:mt-10'>
      <ThemeProvider theme={mdTheme}>
        <div>
          ...
          {tab === 5 && (
            <TransactionTab index={5} activeTab={tab} setTab={setTab} data={transactionData} />
          )}

          {tab === 6 && <SongReqTab index={6} activeTab={tab} setTab={setTab} data={songReqData} />}
        </div>

        <Navbar tab={tab} setTab={handleTab} setPrevTab={setPrevTab} />
      </ThemeProvider>
    </div>
  );
};
